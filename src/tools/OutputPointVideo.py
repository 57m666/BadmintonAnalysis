...
def draw_frame(frame_id):
    ...
    if joints:
        ...
        for player_key, color in [('top', 'b'), ('bottom', 'r')]:
            ...
            for edge in [(0, 1), (0, 2), (2, 4), (1, 3), (6, 8), (8, 10), (11, 12),
                         (5, 7), (7, 9), (5, 11), (11, 13), (13, 15), (6, 12),
                         (12, 14), (14, 16), (5, 6)]:
                if edge[0] < len(x) and edge[1] < len(x):
                    if (x[edge[0]], y[edge[0]]) != (0, 0) and (x[edge[1]], y[edge[1]]) != (0, 0):
                        ax.plot([x[edge[0]], x[edge[1]]], [y[edge[0]], y[edge[1]]], color + '-')

    # --- 球 ---
    ball = ball_dict.get(str(frame_id), None)
    if ball:
        ax.scatter(ball['x'], ball['y'], c='g')
        ax.annotate("ball", (ball['x'], ball['y']), textcoords="offset points", xytext=(0,10), ha='center')
